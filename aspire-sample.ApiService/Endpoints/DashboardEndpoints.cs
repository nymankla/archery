...
public static class DashboardEndpoints
{
    ...
    static async Task<IResult> GetDashboard(ArcheryDbContext db, CancellationToken ct)
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var year  = today.Year;

        // Member stats — two COUNT queries
        ...
        // Fee stats — fetch lightweight rows, aggregate in-memory
        ...
        // Competition stats — fetch slim rows, split in-memory
        ...
        // Recent competitions (past, most recent first) with participant counts
        ...
        // Top 5 scorers this year (club members only, non-disqualified)
        ...
        return Results.Ok(new DashboardDto(
            new MemberStatsDto(totalActive, totalInactive, newThisYear),
            new FeeStatsDto(paid, unpaid, partial, noFee,
                totalCollected, totalOutstanding, collectionRatePct),
            new CompetitionStatsDto(
                thisYearCount,
                upcoming.Count,
                nextComp is null ? null : new NextCompetitionDto(nextComp.Name, nextComp.Date, nextComp.Location)),
            topScorers,
            recentComps.Select(c => new RecentCompetitionDto(
                c.Name, c.Date, c.Location, c.Type,
                participantCounts.GetValueOrDefault(c.Id, 0))).ToList()));
    }

    private record DashboardDto(
        MemberStatsDto Members,
        FeeStatsDto Fees,
        CompetitionStatsDto Competitions,
        List<TopScorerDto> TopScorers,
        List<RecentCompetitionDto> RecentCompetitions);

    private record MemberStatsDto(int TotalActive, int TotalInactive, int NewThisYear);

    private record FeeStatsDto(
        int Paid, int Unpaid, int Partial, int NoFee,
        decimal TotalCollected, decimal TotalOutstanding, int CollectionRatePct);

    private record CompetitionStatsDto(
        int TotalThisYear, int UpcomingCount, NextCompetitionDto? NextCompetition);

    private record NextCompetitionDto(string Name, DateOnly Date, string Location);

    private record TopScorerDto(
        string MemberName, BowClass BowClass, int Score, string CompetitionName);

    private record RecentCompetitionDto(
